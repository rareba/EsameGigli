# File        Figure 4.9 Poverty and Education _ US States
#                        With smooth and lollipop residuals
# By          Daniel B. Carr
...
pdf(width=6.5,height=9,file="Fig4_9.pdf")
...
good = rownames(edPov)!= "DC"
edPovNotDC = edPov[good,]
x = edPovNotDC$pov
y = edPovNotDC$ed
edSmooth =loess(y~x,span=.7,degree=2)
ysm= edSmooth$fitted


dot = edPov$ed
dotRange = range(dot)
# expand the range by 11%
...
for (i in 1:nGroups){
   ...
   if(i==6) pen=6
   lines(ysm[gsubs],laby,lwd=1,col="black")
   segments(dot[gsubs],laby,ysm[gsubs],laby)
   points(dot[gsubs],laby,pch=21,
         cex=dotCex,bg=hdColors[pen],col='black')   	   
   points(dot[gsubs],laby,pch=21,
         cex=dotSmallCex,bg="white",col="white")  
}
...
regionBorders = stateVBorders       # more generic script
refRegionBorders = nationVBorders    # more generic script
...
mtext(side=3,line=line1,'The Middle State',cex=cex)

panelSelect(panels,11,4)
panelScale()
mtext(side=1,line=line3,'Cumulative Up To',cex=cex)
mtext(side=1,line=line4,'The Middle State',cex=cex)
...
if(i < 6) frontRegions = regionId[1:iEnd[i]] else 
          frontRegions = regionId[iBegin[i]:51]

if(i==5){gsubs = c(gsubs,26);frontRegions=c(frontRegions,regionId[26])} #median state added here

if(i==7){gsubs = c(gsubs,26);frontRegions=c(frontRegions,regionId[26])} #median state added here
