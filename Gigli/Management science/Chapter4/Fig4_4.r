# File      Fig4_4.r  Pennylvania hazardous air pollutants
# By        Dan Carr
...
# 1. Read county data and extra the PA counties
...
#    county boudary fields are by fips codes.


PAfips = read.table('PA Fips.txt',header=FALSE,row.names=1)

nchr = nchar(triPA[,1])
countyNam = toupper(substring(triPA[,1],1,nchr-4))

triHap = PAfips
triHap$Air = rep(NA,nrow(triHap))
loc = match(rownames(triHap),countyNam)
triHap$Air = triPA[loc,2]
colnames(triHap) = c("Fips","Haps")
triHap[,1] = triHap[,1]+42000
ord = order(triHap[,2])
...
ord = order(triHap[,2],decreasing=TRUE)
triHap = triHap[ord,] 

#5. Open graphics device and set panel layout________________________
pdf(width=6.5,height=6.8,file="Fig4_4AirPollutants.pdf")
...
#   points(rep(.07,nsubs),lady,pch=21,bg=hdcolors[pen],col='black',cex=dcex)
    text(.95,lady,gnams,cex=cex.sm,col='black',adj=c(1,.5))
}
...
for(i in 9:16){
    ...
    panelScale(c(0,1),c(1-ypad,nsubs+ypad))
#    points(rep(.07,nsubs),lady,pch=21,bg=hdcolors[pen],col='black',cex=dcex)
    text(.945,lady,gnams,cex=cex.sm,col='black',adj=c(1,.5))
}

#9 Plot dots__________________________________________

panVal =triHap[,2]/1000000
...
for(i in 1:8){
    ...
    if(i==8){
       ...
       mtext(side=1,line=line2,"Millions of Pounds",cex=cex.axis)
    }
